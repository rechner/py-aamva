...
class AAMVA:
    ...
    def _decodeMagstripe(self, data):
        ...
        dob = datetime.date(int(dobStr[0:4]), int(
            dobStr[4:6]), int(dobStr[6:8]))
    ...
    def _decodeBarcode(self, data):
        ...
        if data[2] == '\x1C':
            ...
            assert data[2] == PDF_RECORDSEP, 'Missing record separator (RS) got (%s)' % repr(
                data[2])
        ...
        if version in (0, 1):
            ...
            for fileId in range(nEntries-1):
                ...
                subfiles_raw.append(data[offset:offset+length])
            ...
            if version == 3:
                decodeFunction = self._decodeBarcode_v3
            if version == 4:
                decodeFunction = self._decodeBarcode_v4
            if version == 5:
                decodeFunction = self._decodeBarcode_v5
            if version == 6:
                decodeFunction = self._decodeBarcode_v6
            if version == 7:
                # FIXME: Seems to only be optional field changes
                decodeFunction = self._decodeBarcode_v8
            if version == 8:
                decodeFunction = self._decodeBarcode_v8
            if version == 9:
                decodeFunction = self._decodeBarcode_v9
    ...
    def _decodeBarcode_v8(self, fields, issueIdentifier):  # 2013 standard
        ...
        # name suffix optional. No prefix field in this version.
        try:
            nameSuffix = fields['DCU'].strip()  # (OPTIONAL 2013 i.)
        except KeyError:
            nameSuffix = None
...
class Weight:
    ...
    def __init__(self, weightRange, weight=None, format='ISO'):
        ...
        if weightRange is None:  # Defined by exact weight (lbs or kg)
            self.exact = True
            assert weight is not None and type(weight) == int, "Invalid weight"
            self.weight = weight
            if self.units == METRIC:
